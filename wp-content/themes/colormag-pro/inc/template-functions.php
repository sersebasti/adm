...
function colormag_header_image_markup( $html, $header, $attr ) {
	...
	if ( ! empty( $header_image ) ) {
		...
		if ( 0 == get_theme_mod( 'colormag_header_image_link', 0 ) && '' != get_theme_mod( 'colormag_header_image_custom_link', '' ) ) {
		...
		if ( 1 == get_theme_mod( 'colormag_header_image_link', 0 ) || '' != get_theme_mod( 'colormag_header_image_custom_link', '' ) ) {
			$output .= '</a>';
		}
...
function colormag_category_id_on_menu( $classes, $item ) {
	...
	if ( '' == get_theme_mod( 'colormag_category_menu_color', '' ) ) {
		return $classes;
	}
