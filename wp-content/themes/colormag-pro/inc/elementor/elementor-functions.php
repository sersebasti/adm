...
if ( ! function_exists( 'colormag_elementor_widgets_meta' ) ) :
	...
	function colormag_elementor_widgets_meta( $meta = array() ) {
		...
		<div class="tg-module-meta <?php echo esc_attr( $human_diff_time ); ?>">
			...
			<?php if ( empty( $meta ) || in_array( 'date', $meta, true ) ) : ?>
				...
				if ( 'post_human_readable_date' === get_theme_mod( 'colormag_post_meta_date_setting', 'post_date' ) ) {
					printf(
						...
						_x( '<span class="tg-post-date posted-on human-diff-time-display">%s ago</span>', '%s = human-readable time difference', 'colormag' ),
						human_time_diff(
							get_the_time( 'U' ),
			...
			<?php if ( empty( $meta ) || in_array( 'reading_time', $meta, true ) ) : ?>
