...
class colormag_ticker_news_widget extends ColorMag_Widget {
	...
	public function __construct() {
		...
		$this->widget_cssclass             = 'widget_ticker_news_colormag widget_featured_posts';
		...
		$this->settings                    = array(
			'widget_layout'            => array(
				...
				'image_url' => get_template_directory_uri() . '/img/style-7.jpg',
			),
			'title'                    => array(
				'type'    => 'text',
				'default' => '',
	...
	public function widget( $args, $instance ) {
		...
		if ( ( true === $popup_link ) && ( is_active_widget( false, false, $this->id_base ) || is_customize_preview() || colormag_elementor_active_page_check() ) ) {
		...
		<div id="category_slider_<?php echo esc_attr( $this->id ); ?>" class="image-ticker-news"
		...
		>
			...
			while ( $get_featured_posts->have_posts() ) :
				...
				<div class="single-article clearfix">
					<?php
					if ( true === $popup_link ) {
						echo '<a href="' . esc_url( get_the_permalink() ) . '" data-fragment="#content" class="colormag-ticker-news-popup-link">';
					}
