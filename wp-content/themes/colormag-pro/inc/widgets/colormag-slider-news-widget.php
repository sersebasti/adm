...
class colormag_slider_news_widget extends ColorMag_Widget {
	...
	public function __construct() {
		...
		$this->widget_cssclass    = 'widget_slider_news_colormag widget_featured_posts';
		...
		$this->settings           = array(
			'widget_layout'            => array(
				...
				'image_url' => get_template_directory_uri() . '/img/style-6.jpg',
			),
			'title'                    => array(
				'type'    => 'text',
				'default' => '',
		...
		<div class="thumbnail-slider-news">
			...
			while ( $get_featured_posts->have_posts() ) :
				...
				if ( has_post_thumbnail() ) {
					...
					$big_image .= '<img src="' . get_template_directory_uri() . '/img/thumbnail-big-slider.png">';
					$big_image .= '</a>';

					// For thumbnail slider image.
					$thumbnail_image .= '<a data-slide-index="' . esc_attr( $j ) . '" href="">';
					$thumbnail_image .= '<img src="' . get_template_directory_uri() . '/img/thumbnail-small-slider.png">';
					$thumbnail_image .= '<span class="title">' . esc_html( $title_attribute ) . '</span>';
					$thumbnail_image .= '</a>';
				}

				// Large slider image display.
				$big_image_output .= '<li class="single-article">';
				...
				$big_image_output .= '<div class="article-content">';
				$big_image_output .= colormag_colored_category( false );
				$big_image_output .= '<h3 class="entry-title">';
				$big_image_output .= '<a href="' . esc_url( get_permalink() ) . '" title="' . esc_attr( $title_attribute ) . '">';
				$big_image_output .= esc_html( colormag_get_the_title( get_the_title() ) );
				$big_image_output .= '</a>';
				$big_image_output .= '</h3>';
				...
				if ( $i == $number || $i == $post_count ) {
					...
					<div id="style6_pager_<?php echo esc_attr( $this->id ); ?>" class="thumbnail-slider">
