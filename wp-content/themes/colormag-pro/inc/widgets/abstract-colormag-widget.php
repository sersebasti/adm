...
abstract class ColorMag_Widget extends WP_Widget {
	...
	public function widget_title( $title, $type, $category, $view_all_button ) {
		...
		if ( 'true' == $view_all_button && ( ! empty( $category ) && 'latest' != $type ) ) {
			$category_link = '<a href="' . esc_url( get_category_link( $category ) ) . '" class="view-all-link">' . esc_html( get_theme_mod( 'colormag_view_all_text', __( 'View All', 'colormag' ) ) ) . '</a>';
		}

		// Display the title.
		echo '<' . apply_filters( 'colormag_widget_title_markup', 'h3' ) . ' class="widget-title" ' . $border_color . '><span ' . $title_color . '>' . esc_html( $title ) . '</span>' . $category_link . '</' . apply_filters( 'colormag_widget_title_markup', 'h3' ) . '>'; // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped
	...
	public function the_title() {
		echo '<' . apply_filters( 'colormag_front_page_widget_post_title_markup', 'h3' ) . ' class="entry-title">' ?>
	...
	public function entry_meta() {
		...
		<?php if ( 1 == get_theme_mod( 'colormag_reading_time_setting', 0 ) ) { ?>
