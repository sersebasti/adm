...
class DUPX_DBInstall
{
    ...
    protected function prepareDB()
    {
        ...
        DUPX_Log::info("--------------------------------------");
        DUPX_Log::info('TABLES AND ROWS IN ARCHIVE');
        DUPX_Log::info("--------------------------------------");
        $archiveTablesList = (array) DUPX_ArchiveConfig::getInstance()->dbInfo->tablesList;
        foreach ($archiveTablesList as $tableName => $tableData) {
            $rows  = ($tableData->insertedRows === false ? $tableData->inaccurateRows : $tableData->insertedRows);
            $label = ($tableData->insertedRows === false ? 'INACCURATE' : 'INSERTED' );
            DUPX_Log::info('TABLE '.str_pad(DUPX_Log::varToString($tableName), 50, '_', STR_PAD_RIGHT).'['.$label.' ROWS:'.str_pad($rows, 6, " ", STR_PAD_LEFT).']');
        }
        DUPX_Log::info("--------------------------------------\n\n");
            ...
            if ($oldPrefix === $newPrefix) {
                ...
                foreach ($tables as $table => $tableInfo) {
                    if (strpos($table, $oldPrefix) === 0) {
                        ...
                        if ($addNotices) {
                            $msg = 'The table "'.$table.'" don\'t have prefix "'.$oldPrefix.'" so the prefix can\'t be changed';
                            DUPX_LOG::info('TABLE PREFIX RENAME: '.$msg);
                            DUPX_NOTICE_MANAGER::getInstance()->addBothNextAndFinalReportNotice(array(
                                'shortMsg'    => 'Table prefix change problem',
                                'level'       => DUPX_NOTICE_ITEM::HARD_WARNING,
                                'longMsg'     => $msg.'<br>',
                                'longMsgMode' => DUPX_NOTICE_ITEM::MSG_MODE_HTML,
                                'sections'    => 'database'
                                ), DUPX_NOTICE_MANAGER::ADD_UNIQUE_APPEND, 'tample_raname_prefix_problem');
                        }
                    }
                }
                $sreachRegEx  = array(
                ...
                if ($addNotices) {
                    DUPX_LOG::info('TABLE PREFIX REGEX SEARCH:'.DUPX_Log::varToString($sreachRegEx), DUPX_Log::LV_DETAILED);
                    DUPX_LOG::info('TABLE PREFIX REGEX REPLACE:'.DUPX_Log::varToString($replaceRegEx), DUPX_Log::LV_DETAILED);
                }
            }
        }
