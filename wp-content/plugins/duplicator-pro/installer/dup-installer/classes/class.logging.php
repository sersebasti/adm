...
class DUPX_Log
{
    ...
    {
        if ($logging > self::$logLevel || !isset($GLOBALS['LOG_FILE_HANDLE']) || !is_resource($GLOBALS['LOG_FILE_HANDLE'])) {
            return;
        }
     ...
     */
    public static function setPostProcessCallabck($callback)
    {
    ...
    public static function resetTime($logging = self::LV_DEFAULT, $fileInfo = true)
    {
        ...
        $file    = $callers[0]['file'];
        $line    = $callers[0]['line'];
        DUPX_Log::info('LOG-TIME'.($fileInfo ? '['.$file.':'.$line.']' : '').' RESET TIME', $logging);
    }
    ...
    public static function logTime($msg = '', $logging = self::LV_DEFAULT, $fileInfo = true)
    {
        ...
        if ($fileInfo) {
            ...
            DUPX_Log::info(sprintf('LOG-TIME[DELTA:%10.5f] ', microtime(true) - self::$microtimeStart).(empty($msg) ? '' : ' MESSAGE:'.$msg), $logging);
        }
    }
    ...
    public static function error($errorMessage)
    {
        ...
        // @todo this doesn't have to be here, in case the parameters are not initialized it generates a fatal error. this function is abstract and can also be called at boot time so this is an error.
        try {
            ...
            // tmp fix in case of error but thsi have to removed
        }
        catch (Error $e) {
            // tmp fix in case of error but thsi have to removed
        }
    ...
    private static function getMessage($errno, $errstr, $errfile, $errline)
    {
        ...
        if (self::$codeReference) {
            ...
            if (DUPX_Log::isLevel(DUPX_Log::LV_DEBUG)) {
                DUPX_Log::info(DUPX_Log::traceToString(debug_backtrace(), 1));
            }
        }
