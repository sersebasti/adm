...
class DUPX_Server
{
    ...
    public static function getWpAddonsSiteLists()
    {
        ...
        foreach ($pathsToCheck as $mainPath) {
            DupProSnapLibIOU::regexGlobCallback($mainPath, function ($path) use (&$addonsSites) {
                if (DupProSnapLibUtilWp::isWpHomeFolder($path)) {
                    $addonsSites[] = $path;
                }
            }, array(
                'regexFile' => false,
    ...
    {
        $absPathNew = DUPX_Paramas_Manager::getInstance()->getValue(DUPX_Paramas_Manager::PARAM_PATH_WP_CORE_NEW);
