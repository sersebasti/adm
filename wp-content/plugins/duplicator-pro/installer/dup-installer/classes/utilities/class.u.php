...
class DUPX_U
{
    ...
    public static function get_default_chunk_size_in_byte($min_chunk_size = '')
    {
        ...
        if (empty($min_chunk_size))
            $min_chunk_size                    = 2 * DUPLICATOR_PRO_INSTALLER_MB_IN_BYTES; // 2 MB;
    ...
    {
        if (empty($min_chunk_size))
            $min_chunk_size = 10 * DUPLICATOR_PRO_INSTALLER_MB_IN_BYTES; // 10 MB;
    ...
    {
        $pathNew = DUPX_Paramas_Manager::getInstance()->getValue(DUPX_Paramas_Manager::PARAM_PATH_NEW);
