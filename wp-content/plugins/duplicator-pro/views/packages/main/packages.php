...
<style>
    ...
    /*Recommeded text*/
    .dpro-inline-block {display:inline-block; float:left;}
    .dpro-recommended-clear-container {clear:both; padding: 10px 0;}
    ...
    .dpro-cursor-pointer { cursor: pointer; }
    ...
    nav.dpro-bar-menu div span i.fa, 
    nav.dpro-bar-menu div span i.fas,
    nav.dpro-bar-menu div span i.far,
    nav.dpro-dnload-menu div span i.fa, 
    nav.dpro-dnload-menu div span i.fas,
    nav.dpro-dnload-menu div span i.far {display:inline-block; width:18px;}
...
<form id="form-duplicator" method="post">
    ...
    <table class="dpro-edit-toolbar">
        <tr>
            <td>
                ...
                <a href="<?php echo DUP_PRO_CTRL_import::getImportPageLink(); ?>" id="btn-logs-dialog" class="button " title="<?php DUP_PRO_U::esc_attr_e("Import") ?>"><i class="fas fa-arrow-alt-circle-down"></i></a>
                <?php
                // DISPLAY GIFT BUTTON
    ...
    <div id="dup-pro-fixes" class="error" style="display: <?php echo $recommended_text_fix_present ? 'block' : 'none' ?>">
        ...
        if ($recommended_text_fix_present) {
            ...
            echo '</p>';
            echo '<div class="dpro-auto-conf">';
            foreach ($system_global->recommended_fixes as $fix) {
                if ($fix->recommended_fix_type == DUP_PRO_Recommended_Fix_Type::Text) {
                    echo "<div><i class='fa fa-question-circle color-alert' data-tooltip='".esc_attr($fix->error_text)."'></i>&nbsp; ".$fix->parameter1." </div>";
                } else if ($fix->recommended_fix_type == DUP_PRO_Recommended_Fix_Type::QuickFix) {
                    echo '<div id="quick-fix-'.$fix->id.'">'  
                            .'<div id="quick-fix-'.$fix->id.'-table" class="dpro-inline-block">'
                                .'<button id="quick-fix-'.$fix->id.'-button" class="dup-pro-quick-fix" type="button" class="button button-primary" data-param="'.$fix->parameter2.'" data-id="'.$fix->id.'" data-toggle="#quick-fix-'.$fix->id.'">'
                                    ."<i class='fa fa-wrench' aria-hidden='true'></i>&nbsp; "
                                    .DUP_PRO_U::__('Resolve This')
                                .'</button>'
                            .'</div>'
                            .'<div id="quick-fix-'.$fix->id.'-message" class="dpro-inline-block" style="margin: 5px 0px 5px 20px;">'
            ...
            echo "</div>";
            echo '<div class="dpro-recommended-clear-container"><a href="#" class="button button-secondary" onclick="DupPro.Pack.ClearMessages();">'.DUP_PRO_U::__('Clear').'</a></div>';
        }
        ?>
    </div>
    <!-- ====================
    ...
    <table class="widefat dpro-pktbl striped">
        ...
        function tablePackageRow($Package)
        {
            ...
            $isRecoveable   = !DUP_PRO_CTRL_recovery::isDisallow() && DUP_PRO_Package_Recover::isPackageIdRecoveable($Package->ID);
            $isRecoverPoint = !DUP_PRO_CTRL_recovery::isDisallow() && DUP_PRO_Package_Recover::getRecoverPackageId() === $Package->ID;
            ...
            $archiveDownloadInfoJson   = DupProSnapJsonU::json_encode_esc_attr($Package->getPackageFileDownloadInfo(DUP_PRO_Package_File_Type::Archive));
            $installerDownloadInfoJson = DupProSnapJsonU::json_encode_esc_attr($Package->getInstallerDownloadInfo());
            ...
            if (($Package->Status >= DUP_PRO_PackageStatus::COMPLETE) || ($storage_problem)) :
                ?>
                <!-- COMPLETE -->
                    ...
                    <td class="inst-name">
                        ...
                        switch ($global->installer_name_mode) {
                            case DUP_PRO_Global_Entity::INSTALLER_NAME_MODE_SIMPLE:
                                ...
                                $installerToolTipTitle = sprintf(DUP_PRO_U::__('Using standard installer name. To improve security, switch to hashed change in <a href="%1$s">%2$s</a>'), get_admin_url(null, 'admin.php?page=duplicator-pro-settings&tab=package'), DUP_PRO_U::__('Settings'));
                                break;
                        ...
                        <span data-dup-pro-copy-value="<?php echo $installerName; ?>" ><i class='far fa-copy dpro-cursor-pointer'></i>
                    </td>
                    <td class="get-btns">
                        <!-- MENU DOWNLOAD -->
                        ...
                        <nav class="dpro-bar-menu">
                            ...
                            <nav class="dpro-bar-menu-items">
                                ...
                                <?php else : ?>
                                    <div class="dpro-btn-package-remote-no-storage disabled" style="color:#999" title="<?php DUP_PRO_U::esc_attr_e("No Remote Storages") ?>">
                                        <span><i class="fas fa-database fa-sm" ></i> <?php DUP_PRO_U::esc_html_e("Storage") ?></span>
                                    </div>
                                <?php
                                endif;
                                ...
                                if ($isRecoveable) {
                                    ?>
                                    <div class="dpro-btn-set-recovery" data-package-id="<?php echo $Package->ID; ?>">
                                        <span><i class="fas fa-undo-alt"></i> <?php DUP_PRO_U::esc_html_e("Set Recovery Point") ?></span>
                                    </div>
                                    <?php
                                }
                ...
                <tr class="<?php echo implode(' ', $rowClasses); ?>" id="duppro-packagerow-<?php echo $Package->ID; ?>">
                    ...
                    <td class="get-btns-transfer" >
                        <?php if ($status >= DUP_PRO_PackageStatus::STORAGE_PROCESSING) : ?>
                            <button id="<?php echo "{$uniqueid}_{$global->installer_base_name}" ?>" <?php DUP_PRO_UI::echoDisabled(!$installer_exists); ?> class="button no-select" onClick="DupPro.Pack.DownloadInstaller(<?php echo $installerDownloadInfoJson; ?>);
                                    return false;">
                                <i class="fa <?php echo ($installer_exists ? 'fa-bolt' : 'fa-exclamation-triangle') ?>"></i> <?php DUP_PRO_U::esc_html_e("Installer") ?>
                            </button>
...
<?php $alert3->showAlert(); ?>
        ...
        $('#dup-pro-fixes').on('click', '.dup-pro-quick-fix',  function() {
            var $this = $(this),
                    pharams = eval('({' + $this.attr('data-param') + '})'),
...
<?php $confirmRestoreBk->showConfirm(); ?>
        ...
        $('.dpro-btn-set-recovery').click(function () {
            DupPro.Pack.SetRecoveryPoint($(this).data('package-id'), function (funcData, data, textStatus, jqXHR) {
                ...
                let icon = $(<?php echo json_encode(DupProSnapLibIOU::getInclude(DUPLICATOR_PRO_PLUGIN_PATH.'/views/tools/recovery/recovery-small-icon.php')); ?>);
                ...
                icon.data('dup-pro-copy-value', funcData.recoveryLink);
