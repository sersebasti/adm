...
class DUP_PRO_Database
{
    ...
    public function build()
    {
        ...
        try {
            ...
            do_action('duplicator_pro_build_database_fail', $this->Package);
    ...
    private function runPHPDump()
    {
        ...
        for (; $dbInsertIterator->valid(); $dbInsertIterator->next()) {
            ...
            $page_count = ceil($dbInsertIterator->getCurrentRows() / $global->package_phpdump_qrylimit);
            ...
            for ($i = 0; $i < $page_count; $i++) {
                $result = DupProSnapLibDB::selectUsingPrimaryKeyAsOffset(
                        ...
                        $table,
                        $row_offset,
                        $global->package_phpdump_qrylimit,
                        $row_offset);
                ...
                if (DupProSnapLibDB::numRows($result) > 0) {
                    $first = true;
                    while (($row   = DupProSnapLibDB::fetchAssoc($result))) {
                        ...
                        $totalCount = $dbInsertIterator->nextRow(0, DupProSnapLibIOU::fwriteChunked($handle, $line));
    ...
    private function writeInsertChunk()
    {
        ...
        for (; $dbInsertIterator->valid(); $dbInsertIterator->next()) {
            ...
            if ($this->traceLogEnabled) {
                ...
                DUP_PRO_Log::trace("chunk_size: ".$global->package_phpdump_qrylimit);
            }

            $indexColumns = DupProSnapLibDB::getUniqueIndexColumn($wpdb->dbh, $table);

            while ($dbInsertIterator->isCurrentTableOffsetValid()) {
                $result = DupProSnapLibDB::selectUsingPrimaryKeyAsOffset(
                        $wpdb->dbh,
                        'SELECT * FROM `'.$table.'` WHERE 1',
                        $table,
                ...
                if (DupProSnapLibDB::numRows($result) > 0) {
                    ...
                    while (($row   = DupProSnapLibDB::fetchAssoc($result))) {
                        ...
                        $totalCount = $dbInsertIterator->nextRow(
                            ...
                            DupProSnapLibIOU::fwriteChunked($handle, $line)
                        );
