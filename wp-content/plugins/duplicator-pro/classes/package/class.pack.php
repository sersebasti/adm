...
class DUP_PRO_Package
{
    ...
    public static function get_from_json($json_string)
    {
        ...
        if (property_exists($stdobject, "statusInfo")) {
            $package->statusInfo = (array) $stdobject->statusInfo;
        }
    ...
    public function create_scan_report()
    {
        ...
        try {
            ...
            $this->statusInfo['hasFilteredCoreFolders'] = $this->Archive->hasWpCoreFolderFiltered();
            $this->statusInfo['hasFilteredSiteTables']  = !DUP_PRO_Import_U::hasAllRequiredTables(DUP_PRO_MU::getMode(), !empty($this->Multisite->FilterSites), $this->Database->info);

            $report['ARC']['Status']['CanbeMigratePackage']    = !DUP_PRO_RestoreOnly_Package::isRestoreOnly();
            $report['ARC']['Status']['MigratePackage']         = ($report['ARC']['Status']['CanbeMigratePackage'] && !$this->Archive->ExportOnlyDB && $report['SRV']['WP']['core'] && !$this->statusInfo['hasFilteredSiteTables']) ? 'Good' : 'Warn';
            $report['ARC']['Status']['HasFilteredCoreFolders'] = $this->statusInfo['hasFilteredCoreFolders'];
            $report['ARC']['Status']['HasFilteredSiteTables']  = $this->statusInfo['hasFilteredSiteTables'];
    ...
    public function run_build()
    {
        try {
            ...
            if ($this->build_progress->initialized == false) {
                ...
                $architecture       = DupProSnapLibUtil::getArchitectureString();
            ...
            if ($this->build_progress->has_completed()) {
                ...
                if ($this->build_progress->failed) {
                    /**
                     * @todo remove after major refactoring related to failed package processing
                     */
                    if ($schedule != null) {
                        //$schedule->times_run++;
...
//					$schedule->last_run_status	 = DUP_PRO_Package_Build_Outcome::SUCCESS;
            ...
            /**
             * @todo temp fix ....  remove after major refactoring
             */
            $schedule = DUP_PRO_Schedule_Entity::get_by_id($this->schedule_id);
            if ($schedule != null) {
                $this->post_scheduled_build_processing(0, false);
    ...
    public function build_integrity_check()
    {
        ...
        if (!strstr($sql_done_txt, DUPLICATOR_PRO_DB_EOF_MARKER) ||
    ...
    public static function set_manual_template_from_post($post = null)
    {
        if (isset($post)) {
            ...
            $secure_pass                           = isset($post['secure-pass']) ? DupProSnapLibUtil::sanitize_non_stamp_chars_newline_and_trim($post['secure-pass']) : '';
    ...
    {
        foreach ($this->upload_infos as $upload_info) {
            if ($upload_info->failed || $upload_info->cancelled) {
                return true;
            }
        }
