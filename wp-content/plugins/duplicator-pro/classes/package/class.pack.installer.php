...
class DUP_PRO_Installer
{
    ...
    public function getThemesInfo()
    {
        ...
        if (is_multisite()) {
            foreach (DupProSnapLibUtilWp::getSites() as $site) {
                switch_to_blog($site->blog_id);
    ...
    public function initConfigFiles()
    {
        ...
        foreach ($configFilePaths as $identifier => $path) {
            if ($path !== false) {
                try {
                    $this->origFileManger->addEntry($identifier, $path, DupProSnapLibOrigFileManager::MODE_COPY, self::CONFIG_ORIG_FILE_FOLDER_PREFIX.$identifier);
                }
                catch (Exception $ex) {
