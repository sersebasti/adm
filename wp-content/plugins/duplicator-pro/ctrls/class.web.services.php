...
class DUP_PRO_Web_Services
{
    ...
    function duplicator_pro_package_scan($not_ajax_call = false)
    {
        ...
        try {
            ...
            $json = '{"Status" : 3, "Message" : "'.sprintf(DUP_PRO_U::esc_html__("Fatal Error occurred. Error message: %s"), $ex->getMessage()).'"}';
            die($json);
        }
        ...
        try {
            ...
            if (!$use_curl) {
                ...
                if (!$ftp_connect_exists_filtered) {
                    throw new Exception(sprintf(DUP_PRO_U::esc_html__('FTP storage without use cURL requires FTP module to be enabled. Please install the FTP module as described in the %s.'),
