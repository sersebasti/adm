...
class WonderPlugin_Gridgallery_Update {
	...
	public $controller, $slug, $version, $api_url, $update_response, $wp_version, $product, $plugin, $domain;
		...
		if( $pagenow === 'update-core.php' && !empty($info))
		{
			if ( !isset($info->key_status) || $info->key_status != 'valid')
			{
				...
				else
				{
					...
					{
						echo '<div class="error notice is-dismissible"><p><strong>' . $info->name . '</strong>: To update the plugin in WordPress backend, please <a href="https://www.wonderplugin.com/wordpress-gridgallery/order/" target="_blank">upgrade to a commercial license</a>, otherwise, you will get the <strong>Update package not available</strong> error.</div>';				
					}
					else
					{
	...
	function show_plugin_update_message( $plugin_data, $r ) {
		...
		else
		{
			if (WONDERPLUGIN_GRIDGALLERY_VERSION_TYPE == 'F')
			{
				echo '<br>To get automatic update, please <a href="https://www.wonderplugin.com/wordpress-gridgallery/order/" target="_blank">upgrade to a commercial license</a>.';
			}
			else
			{
